...
export default function ProductsSection() {
  return (
    <section id="products" className="py-24 sm:py-32 px-5 sm:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
          {products.map((product, i) => (
            <motion.div
              ...
              className="glass rounded-3xl overflow-hidden card-glow group relative flex flex-col"
            >
              {/* Image Container */}
              <div className="relative w-full h-56 sm:h-72 overflow-hidden border-b border-primary/5 bg-[#0a0e1a]">
              ...
              <div className="p-6 sm:p-8 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {product.tech.map((t) => (
                    <span
                      key={t}
                      className="px-3 py-1 rounded-full text-xs font-medium bg-secondary/5 border border-primary/5 text-secondary"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <a
                  ...
                  className={`inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-semibold text-white bg-gradient-to-r ${product.color} shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 w-fit`}
                >
                  {product.cta}
